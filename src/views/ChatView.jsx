    ...
    <div className="h-screen flex flex-col bg-gray-900 text-white">
        ...
        <div className="mx-auto max-w-2xl p-4 space-y-4">
          {messages.map((m, idx) => (
            <div key={idx} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div
                className={`rounded-xl px-4 py-2 text-sm md:text-base break-words max-w-full ${m.role === 'user' ? 'bg-green-500 text-white' : 'bg-gray-700 text-white'}`}
              >
                {m.text}
              </div>
            </div>
      ...
      <form onSubmit={handleSubmit} className="border-t border-gray-700 bg-gray-800 p-4">
        <div className="flex mx-auto max-w-2xl gap-2">
          <textarea
            className="flex-1 border border-gray-600 rounded-md p-2 resize-none bg-gray-700 text-white"
            value={input}
